...
18. 1.  For \\(m\in\mathcal{M}\\), define
        ...
        \\(m\_1\\). On receiving the challenge ciphertext \\(c\\), they output 0
        if \\(c\in\mathcal{C}\brak{m\_0}\\) and guess randomly otherwise. Thus,

        \begin{align}
            \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}} = 1} &= \frac{1}{2}\brak{\prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}} = 1\ |\ b = 0} + \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}} = 1\ |\ b = 1}} \\\\
            &= \frac{1}{2}\brak{1 - \epsilon
        \end{align}