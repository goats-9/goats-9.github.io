...
17. Let \\(\mathcal{K} = \cbrak{0,1}^l\\) and \\(\mathcal{M} =
    \cbrak{0,1}^{l+t}\\). The key-generation algorithm \\(\textsf{Gen}\\) picks
    \\(k\random\mathcal{K}\\). Define the encryption algorithm
    \\(\textsf{Enc}\\) to be
    
    \begin{equation}
        \textsf{Enc}\brak{m,k} = m^\prime\oplus k,
    \end{equation}

    where \\(m^\prime\\) denotes the first \\(l\\) bits of \\(m\\). The
    decryption algorithm \\(\textsf{Dec}\\) picks \\(r\random\cbrak{0,1}^t\\)
    and is defined as

    \begin{equation}
        \textsf{Dec}\brak{c,k} = c\oplus k\\|r.
    \end{equation}

    Clearly, for \\(t\ge 1\\), we have \\(\abs{\mathcal{K}} <
    \abs{\mathcal{M}}\\), and the cryptosystem is perfectly secure since it is
    essentially a one-time pad on the first \\(l\\) bits of \\(m\\).

    Consider the event of guessing a randomly chosen message \\(m\\), according
    to any probability distribution over \\(\mathcal{M}\\). Then, by the
    pigeonhole principle, there exists an \\(m\in\mathcal{M}\\) such that

    \begin{align} 
    \abs{\mathcal{M}}^{-1} &\ge \pr{M=m} \\\\
    &= \pr{M=m\ |\ C=c} \label{eq:perf-sec-def} \\\\
    &= \pr{M=m,\ K=k\ |\ C=c} \\\\
    &= \pr{K=k\ |\ C=c}\pr{M=m\ |K=k,\ C=c} \\\\
    &= \abs{\mathcal{K}}^{-1}\pr{\textsf{Dec}\brak{\textsf{Enc}\brak{m,k},k} = m} \ge \abs{\mathcal{K}}^{-1}2^{-t}.
    \end{align}

    (Note that \eqref{eq:perf-sec-def} follows from the definition of perfect
    security.) Hence, we have \\(2^{-t}\abs{\mathcal{M}} \le \abs{\mathcal{K}} <
    \abs{\mathcal{M}}\\).

18. 1.  For \\(c\in\mathcal{C}\\), define

        \begin{equation}
            \mathcal{M}\brak{c} \triangleq \cbrak{c\oplus k\ |\ k \in \mathcal{K}}.
        \end{equation}

        Construct an adversary \\(\mathcal{A}\\) that does the following:
        \\(\mathcal{A}\\) outputs two distinct messages \\(m\_0\\) and
        \\(m\_1\\) chosen uniformly at random from \\(\mathcal{M}\\). On 
        receiving the challenge ciphertext \\(c\\), they output 0 if 
        \\(m\_0\in\mathcal{M}\brak{c}\\) and output 1 otherwise. Thus,

        \begin{align}
            \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}} = 1} &= \sum\_{i=0}^{1}\prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}}=1\ |\ b=i}\pr{b=i} \\\\
            &= \frac{1}{2} + \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}}=1\ |\ b=1}\pr{b=1} \\\\
            &\le \frac{1}{2} + \prs{m\_0\notin\mathcal{M}\brak{c}} \le \frac{1}{2} + \epsilon.
        \end{align}

        Thus, the given cryptosystem is \\(\epsilon\\)-perfectly secure.

    2.  Define for \\(m\in\mathcal{M}\\),

        \begin{equation}
            \mathcal{C}\brak{m} \triangleq \cbrak{m\oplus k\ |\ k\in\mathcal{K}}.
        \end{equation}

        Note that for distinct messages \\(m\_0\\) and \\(m\_1\\),

        \begin{align}
            \abs{\mathcal{C}\brak{m\_1}\setminus\mathcal{C}\brak{m\_0}} &= \abs{\mathcal{C}\brak{m\_1}} - \abs{\mathcal{C}\brak{m\_0}\cap\mathcal{C}\brak{m\_1}} \\\\
            &= \abs{\mathcal{C}\brak{m\_0}\cup\mathcal{C}\brak{m\_1}} - \abs{\mathcal{C}\brak{m\_0}} \\\\
            &= \le 2^l\brak{1-\brak{1-\epsilon}} = 2^l\epsilon
            \label{eq:setminus-card}
        \end{align}

        Consider an adversary \\(\mathcal{A}\\) defined as follows. 
        \\(\mathcal{A}\\) outputs two random distinct messages \\(m\_0\\) and
        \\(m\_1\\). On receiving \\(c\\), \\(\mathcal{A}\\) outputs 0 if
        \\(c\in\mathcal{C}\brak{m\_0}\\) and 1 otherwise. Then, using
        \eqref{eq:setminus-card},

        \begin{align}
            \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}}=1} &= \frac{1}{2}\brak{1 + \prs{\textsf{PrivK}\_{\mathcal{A},\Pi}^{\textsf{eav}}=1\ |\ b=1}} \\\\
            &= \frac{1}{2}\brak{1 + \pr{c\notin\mathcal{C}\brak{m\_0}\ |\ M=m\_1}} \\\\
            &= \frac{1}{2}\brak{1 + \pr{c\notin\mathcal{C}\brak{m\_0}\cap\mathcal{C}\brak{m\_1}}} \\\\
            &= \frac{1}{2}\brak{1 + \frac{\epsilon}{1-\epsilon}} \\\\
            &= \frac{1}{2} + \frac{\epsilon}{2\brak{1-\epsilon}}.
        \end{align}

        Hence, the cryptosystem is 
        \\(\frac{\epsilon}{2\brak{1-\epsilon}}\\)-perfectly secure.
